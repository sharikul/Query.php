	...
	class Query {
        ...
        static function update_where($column = '', $value = '', $table = '', array $options = null) {
            ...
            return self::_query('', $table, array(
                ...
                'where' => $column.' = :value',
                'update' => $update,
                'placeholders' => array_merge(array(':value' => $value), $placeholders)
            ));
        }
        ...
        static function setup( array $options) {
			...
			if( class_exists('PDO')) {
                ...
                if( $driver === 'mysql') {
                    ...
                    die( sprintf('<strong>Error</strong>: Driver "<code>%s</code>" is not supported by Query.php. Check out the <a href="%s">Github Docs</a> for more information on this error.', $driver, '#github'));
                }

			}

            else {
               die( sprintf('<strong>Error</strong>: Query.php requires the PDO module to be installed. Check out the <a href="%s">Github Docs</a> for more information on this error.', '#github'));
            }
				...
				if( substr($key, 0, 4) === substr($query, 0, 4)) {
					...
					for( $i = 1; $i <= count($matches); $i++ ) {
                        ...
                        if( !empty($findings)) {
                            ...
                            echo sprintf('Unrecognised expression: <strong>%s</strong>. Visit <a href="%s">%s</a> for assistance.', $query, '#github-link', '#github-link');
                            exit();
                        }
					}
